...
# ---- Single NAT gateway (cost trade-off; HA would use one per AZ) ----
...
resource "aws_security_group" "api" {
  ...
  description = "API tasks; ingress only from ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "redis" {
  ...
  description = "Redis; ingress only from api and worker"
  vpc_id      = aws_vpc.main.id
...
resource "aws_service_discovery_private_dns_namespace" "main" {
  ...
  description = "Internal DNS for service discovery"
  vpc         = aws_vpc.main.id

  tags = {
    Name = "${var.project_name}-cloud-map"
  }
}
